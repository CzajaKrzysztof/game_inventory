...
def display_inventory(inventory):
    ...
    print("Total number of items: {0}".
          format(get_total_items_count(inventory)))
...
def get_sorted_inventory(inventory, order):
    ...
    if order == "count,asc":
        inventory_sorted = sorted(list(inventory.items()),
                                  key=lambda item: item[1])
    elif order == "count,desc":
        inventory_sorted = list(reversed(sorted(list(inventory.items()),
                                         key=lambda item: item[1])))
    elif order is None:
...
def print_table(inventory, order=None):
    ...
    print("Inventory:\n")
    print("{0:>{width_c}}{1:>{width_i}}".
          format("Count", "item name", width_c=max_value_length + 4,
                 width_i=max_key_length + 3))
    print("-" * (max_key_length + max_value_length + 7))
    for i in sorted_inventory:
        print("{0:>{width_c}}{1:>{width_i}}".
              format(i[1], i[0], width_c=max_value_length + 4,
                     width_i=max_key_length + 3))
    print("-" * (max_key_length + max_value_length + 7))
    print("Total number of items: {0}".
          format(get_total_items_count(inventory)))
